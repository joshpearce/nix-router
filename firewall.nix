        ...
        ruleset = ''
          table ip filter {
            ...
            chain input {
              ...
              iifname { "${hazmatVlan}" } udp dport { mdns } counter accept comment "Allow mdns from hazmat so kids can print"
              iifname { "${iotVlan}" } udp dport { 53 } counter accept comment "Allow dns from iot to local dns proxy"
              iifname { "${iotVlan}" } ip saddr { 10.13.93.50 } tcp dport { 3493 } counter accept comment "Allow home assistant to talk to router UPSD"
            ...
            chain forward {
              ...
              iifname { "${lanVlan}", "${hazmatVlan}", "${k8sVlan}" } oifname { "${iotVlan}" } counter accept comment "Allow trusted LAN to IoT"
              iifname { "${iotVlan}" } oifname { "${lanVlan}", "${hazmatVlan}", "${k8sVlan}", "${tsIface}" } ct state { established, related } counter accept comment "Allow established from iot back to LANs"
